...
## Graphics and music continuation (2026-09-19)

- Fixed wall tiling overshoot: final tiles use cropped source rectangles and terminate at exact wall bounds.
- Reused complete prop regions from `assets/generated_atlas/biomech_props_atlas_v1_alpha.png` rather than the tightly clipped v2 standalone exports. Runtime crops include transparent margins; prop/terminal proportions are preserved. No source bitmap was modified.
- All four level files have corrected prop positions. Rotated decor extents are checked against walls, door apertures and playable bounds. Signs search for clear space; props have smaller, consistent footprints. Enemy visuals are 48x37.5 with a 30.5 navigation radius, and one research-spine spawn was moved clear of its wall.
- `music.rs` starts two persistent looping ACE-Step players, uses line-of-sight threat detection with a six-second hold, and fades volumes. F6 toggles music, F7/F8 change music volume; preferences now persist through `audio_settings.rs` (see continuation below). Default music volume is 32%, reduced in menus/pause/death.
- Generated actual ACE-Step 1.5 turbo masters through the existing local worker (not dryrun). Runtime Vorbis files: `assets/music/abyssal-circuit.ogg` (65.45s) and `quarantine-pulse.ogg` (69.19s). `assets/music/manifest.json` records original prompts, generated captions, task IDs, seeds, hashes and edits. Local masters/full worker metadata are Git-ignored, with worker source paths retained in the manifest.
- Audio validation: 48 kHz stereo, normalized to -22 dB RMS, peaks below -3 dB, no silence of 0.5s or longer below -50 dB. Loop edits use one-bar wrap crossfades. Subjective musical preference still needs the user's listening feedback.
- Validation: 81 unit tests (72 core + 9 game), content/entry validation, graphical captures under `/tmp/eutherbreed-art-<level>/`. Visual review mode supports `--visual-level <level>` and a full-map overview; it uses a separate save slot. Existing Vulkan SPIR-V validation chatter remains host-specific.

## Terminal feedback and persistent audio settings (2026-09-19)

- Preserve exploration: subtle terminal display/sample changes, brief response pulses, sector lighting change and quiet ventilation after required objectives complete. No new directions or repeated-use notices.
- Terminal interaction selects the nearest terminal within 42 units. Already-used state takes priority over missing bio. New original synthesized analysis/used/denied cues live in `assets/audio`, generated by `scripts/generate-terminal-audio.py`; ventilation is a periodic four-second loop.
- Objective energy barriers release after analysis even at a distance. Keycard doors remain proximity-operated. Blocked doors give a brief indicator flash and denial cue on E. Existing collision/open animation lifecycle is retained.
